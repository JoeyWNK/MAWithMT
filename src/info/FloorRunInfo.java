...
public class FloorRunInfo {
	...
	public static void floorRunInfo(Document doc) throws Exception{
		...
		if(event_type.equals("1")){
			...
			if((boolean)xpath.evaluate("count(//explore/special_item/before_count)>0", doc, XPathConstants.BOOLEAN))
			Process.info.gather = Integer.parseInt(xpath.evaluate("//explore/special_item/before_count", doc));
			if((boolean)xpath.evaluate("count(//explore/special_item/item_id)>0", doc, XPathConstants.BOOLEAN))
			Process.info.gatherID = Integer.parseInt(xpath.evaluate("//explore/special_item/item_id", doc));
		}else{
		...
		Process.info.nextExp = Integer.parseInt(xpath.evaluate("//explore/next_exp", doc));
